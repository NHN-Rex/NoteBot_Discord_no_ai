...
def load_data(data):
    ...
    # Làm sạch cột TIỀN:
    df['TIỀN'] = df['TIỀN'].str.replace(r'[₫\s]', '',
                                        regex=True)  # bỏ ₫ và khoảng trắng
    df['TIỀN'] = df['TIỀN'].str.replace(',', '',
                                        regex=False)  # bỏ dấu phẩy hàng nghìn
...
def total_debt_by_person_in_month(data, month=None, year=None):
    ...
    for recipients in data['NGƯỜI NHẬN']:
        if recipients in known_names:
            ...
            all_names.extend(
                recipients.split(' '))  # giả sử mỗi tên cách nhau bằng dấu ' '
    ...
    for p in unique_names:  # p là payer
        for r in unique_names:  # r là receiver
            if p != r and (p, r) not in done_pairs and (r,
                ...
                total_pay_by_p = df_filtered[df_filtered['NGƯỜI CHI'] ==
                                             p]['TIỀN'].sum()
                total_pay_by_r = df_filtered[df_filtered['NGƯỜI CHI'] ==
                                             r]['TIỀN'].sum()
    ...
    df = pd.DataFrame(
        [(p[0], p[1], r)
         for p, r in debt_by_person.items() if p[1] != 'Mọi Người'],
        columns=['Chủ nợ', 'Người nợ', 'Số tiền'])
    df_group_detail = df.groupby(
        ['Người nợ',
         'Chủ nợ'])['Số tiền'].sum().reset_index().sort_values(by='Người nợ')
...
def generate_chart_debt(name, data):
    ...
    # Người mà name nợ
    df_person_r = df_debt[df_debt['Người nợ'] == name][['Chủ nợ',
                                                        'Số tiền']].copy()
    ...
    # Người mà nợ name
    df_person_p = df_debt[df_debt['Chủ nợ'] == name][['Người nợ',
                                                      'Số tiền']].copy()
    ...
    # Gộp lại
    df_all = pd.concat([
        df_person_r[['Tên người', 'Giá trị']],
        df_person_p[['Tên người', 'Giá trị']]
    ])
    ...
    for p in ax.patches:
        ...
        if height != 0:
            ax.text(p.get_x() + p.get_width() / 2.,
                    height + (5_000 if height > 0 else -5_000),
                    f'{int(height):,}',
                    ha='center',
